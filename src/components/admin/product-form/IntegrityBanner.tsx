...
import { AlertTriangle, CheckCircle2, RefreshCw, Zap, ShieldAlert, Database, Search } from 'lucide-react';
...
export const IntegrityBanner = ({ productId, uiVariants }: IntegrityBannerProps) => {
  ...
  const startScan = async () => {
    setLoading(true);
    const data = await runDeepScan(productId);
    setResults(data);
    setLoading(false);
  };
  ...
  return (
    <div className={cn(
      ...
      <div className="flex flex-col gap-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 font-mono">UUID: {productId}</p>
            </div>
          </div>
        ...
        {/* Grade de Estratégias */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {results.map((res, i) => (
            <div key={i} className={cn(
              "p-5 rounded-3xl border transition-all",
              res.success ? "bg-green-50 border-green-100" : "bg-gray-50/50 border-gray-100"
            )}>
              <div className="flex items-center justify-between mb-3">
                <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">{res.strategy}</span>
                {res.success ? <CheckCircle2 size={16} className="text-green-500" /> : <Database size={16} className="text-gray-200" />}
              </div>
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-2xl font-bold">{res.count}</p>
                  <p className="text-[10px] text-gray-400 uppercase font-bold">Variações</p>
                </div>
        ...
        {mismatch && (
          <div className="bg-red-500 text-white p-6 rounded-[32px] flex items-center gap-6 animate-pulse">
            <ShieldAlert size={32} />
            <div>
              <p className="font-bold uppercase tracking-[0.1em]">Ponte Quebrada Detectada</p>
              <p className="text-xs opacity-90 leading-relaxed">
                O banco confirmou dados via <span className="underline font-bold">Relacionamento</span>, mas a UI não os exibiu. 
                Isso confirma que o código de renderização está ignorando os dados carregados.
              </p>
            </div>
          </div>
        )}
